...
prf_sinistros <- open_dataset(temp_file)|>
   mutate(
...
rtdeaths |> 
  ...
  plot_ly(x = ~ano_ocorrencia, y = ~obitos, 
          ...
          hoverinfo = "text") |> 
  plotly::layout(yaxis = list(exponentformat = "none",
                              title = "Óbitos"),
...
plot_pred_tri <- pred_trimestre_2024 |> 
...
plot_qq_tri <- pred_trimestre_2024 |> 
...
plot_pred_mensal <- pred_mensal_2024 |> 
  filter(year(data) > 2010) |> 
  pivot_longer(cols = c(.pred, mortes),
               names_to = "Tipo",
               values_to = "Mortes") |> 
...
plot_qq_mensal <- pred_mensal_2024 |> 
  select(data, .pred, mortes) |> 
  drop_na() |> 
  rename(Previsto = .pred, Real = mortes) |> 
  ggplot(aes(x = Real, y = Previsto)) +
...
```{r}
#| echo: false
#| label: fig-pred-arima 
#| fig-cap: Previsão por SARIMA
...
<!-- IPCA entre 12/2014 e 12/2023: 68,14% -->
...
Os componentes de custos associados às pessoas que foram à óbito por consequência de sinistros de trânsito englobam os custos pré-hospitalares, custos hospitalares, custos de perda de perda de produção e custos de remoção. O IPEA estimou um custo total associado à cada vítima fatal de sinistros de R\$ 433.286,69, considerando um valor em reais de dezembro de 2014 [@carvalhoCustosAcidentesTransito2020a]. Com o objetivo desse atualizar esse valor para dezembro de 2023, utilizou-se o Índice Nacional de Preços ao Consumidor Amplo (IPCA). De acordo com a Calculadora do Cidadão, disponibilizada pelo @bancocentraldobrasilCalculadoraCidadao2022, o IPCA acumulado entre dez/2014 e dez/2023 é de 68,14%. Assim, para alcançar o custo por vítima fatal em 2023, multiplica-se a taxa de `r scales::number(ipca, decimal.mark = ",", accuracy = 0.0001)` pelo custo de 2014, alcançando um **custo por óbito de `r custo_individual`**.

Por fim, multiplicando o custo individual das mortes em 2023 pela quantidade estimada de óbitos de `r scales::number(pred_2023, big.mark = ".", decimal.mark = ",")`, tem-se que o custo total dos óbitos previstos no trânsito em 2023 é de `r custo_rs`, ou aproximadamente **R\$ 25,2 bilhões**.
...
leitos_path <- here("data", "leitos_2023.csv")
leitos <- read_csv2(leitos_path)
